...
cannot answer leaves the rule unanchored and pages. An unpriced book defers the anchor to
the next pass (docs/paper-week/day-5-readiness.md, §3.5).

`equity_curve` growing without bound is untouched and still open.

### 4. `client_order_id` is random, not deterministic

`order.py:77` generates it with `uuid.uuid4()`. Both CLAUDE.md §1.4 and the `Order` docstring
...
`PositionSizeSpec.value` (`strategy/rules.py:142`) is a bare `Decimal`. Nothing rejects
...
`RiskSpec.flatten_at_close` (`strategy/rules.py:172`) exists and is never referenced
...
- **`RuleSet.max_concurrent_positions`** (default 5, `strategy/rules.py:188`) is a
