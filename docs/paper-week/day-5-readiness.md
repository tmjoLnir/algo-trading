...
>   restore and *before* `catch_up_on_orders`. It adopts the restored stops into `_protective`
>   (a stop price, a venue id, not terminal, and not `purpose=entry`, so a stop-entry is never
>   counted as cover). They are the runner's own instances, which trade updates mutate, so a
>   stop that fires or is cancelled leaves the router's count too. A stop the book did not
>   restore is still an orphan, and an orphan quarantines before this can matter.
> - **F1.** A symbol not in the last page now pages at once, cooldown or not. While anything is
>   naked, the set re-pages once per cooldown as a "Still N…" rollup, carrying how long the
>   oldest has gone without a stop.
> - **F2.** `SubmitResult.refusal_reason` falls back to the order's `reject_reason` when the
>   decision's is empty, and `runner.position_unprotected` reads it.
>
> The same adoption closes §4.5. `_cancel_stale_protection` still does not read the venue
